...
# Ele é uma sequência de passos que começa com a criação de um vetor de dados e
# depois

# 1. Calcula uma série de estatísticas desse vetor
# 2. Imprime essas estatísticas em texto para que você possa colar isso em outro
# lugar
...
arquivos <- c("dados/voos_de_janeiro.csv", "dados/voos_de_fevereiro.csv", 
              ...
              "dados/voos_de_abril.csv", "dados/voos_de_maio.csv",
...
for(arquivo in arquivos){
  ...
  if(media > meta){
    ...
    mensagem_final <- paste("No arquivo ", arquivo, " a média do atraso foi de",
                            media,
          "minutos, o que é maior do que a meta de", meta, "minutos.")
    
  } else if(media <= meta) {
    
    mensagem_final <- paste("[ALERTA - NÃO BATEU A META] No arquivo ",
                            arquivo, " a média do atraso foi de", media,
...
# 2. Adapte o script para que ele tome a média em todos os voos, 
# não mais em só nos que tem origem JFK e destino ATL
